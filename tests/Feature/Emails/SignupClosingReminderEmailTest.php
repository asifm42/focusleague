...
class SignupClosingReminderEmailTest extends TestCase
{
    ...
    function users_not_signed_up_for_the_current_cycle_get_a_reminder()
    {
        ...
        Mail::assertSent(SignupClosingReminderEmail::class, function ($mail) use ($user2) {
            return $mail->hasTo($user2->email);
        });

        Mail::assertSent(SignupClosingReminderEmail::class, function ($mail) use ($user3) {
            return $mail->hasTo($user3->email);
        });

        Mail::assertSent(SignupClosingReminderEmail::class, function ($mail) use ($user1) {
